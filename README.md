...
| `the server is closing` | From `mcp`: a tool call that had not yet used the database when the server shut down, because the client closed the server's input or the server got `SIGINT` or `SIGTERM`. The call ran nothing, and a connection it had just opened was closed. Start the server again and repeat the call. |
