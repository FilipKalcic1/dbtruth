...
export async function copyOfFixture(t: TestContext): Promise<{ name: string; url: string; sql: (statement: string) => Promise<void> }> {
  ...
  return { name, url, sql: (statement) => once(url, statement) };
}
